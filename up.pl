...
$sql //= 'select simple.* from simple where close > 2 && cap > 20 and date=(select max(date) from simple) order by change_pct desc limit 30';
if ($sql ) {
...
#    print $cgi->Tr($cgi->td($row));
    ...
    for my $key (@$cols) {
        my $value = $row->{$key};
        if ($key eq 'ticker') {
            $value = "<a href='http://finance.yahoo.com/q?s=$value' target=_blank> $value </a>";
        }
        print $cgi->td($value);
    }
    print $cgi->end_Tr();
